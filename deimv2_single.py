...
class LayerScale(nn.Module):
    def __init__(self, dim: int, init_values: float = 1e-5, inplace: bool = False, device=None):
        super().__init__()
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None,
                 act_layer=nn.GELU, drop=0., bias=True, device=None):
        super().__init__()
...
class RopePositionEmbedding(nn.Module):
    def __init__(self, embed_dim, *, num_heads, base=100., min_period=None,
...
class SelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, proj_bias=True,
                 attn_drop=0., proj_drop=0., mask_k_bias=False, device=None):
        super().__init__()
    ...
    def compute_attention(self, qkv, rope=None):
        ...
        q = qkv3[:, :, 0, :].reshape(B, N, self.num_heads, head_dim).transpose(1, 2)  # (B, H, N, D)
    ...
    def forward_list(self, x_list, rope_list=None):
        ...
        att_out = [self.compute_attention(qkv, rope=rope)
...
class SelfAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_ratio=4., qkv_bias=False, proj_bias=True,
                 ffn_bias=True, drop=0., attn_drop=0., init_values=None, drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_class=SelfAttention,
                 ffn_layer=Mlp, mask_k_bias=False, device=None):
        super().__init__()
        ...
        self.mlp = ffn_layer(in_features=dim, hidden_features=int(dim * ffn_ratio),
...
_DINOV3_CONFIGS = {
    'dinov3_vits16': dict(
        ...
        norm_layer="layernormbf16", ffn_layer="mlp", ffn_bias=True,
        proj_bias=True, n_storage_tokens=4, mask_k_bias=True,
    ),
}
_NORM_LAYERS = {
...
class DinoVisionTransformer(nn.Module):
    def __init__(self, name):
        ...
        self.patch_embed = PatchEmbed(img_size=cfg['img_size'], patch_size=cfg['patch_size'],
                                      in_chans=cfg['in_chans'], embed_dim=embed_dim, flatten_embedding=False)
        ...
        self.blocks = nn.ModuleList([
            SelfAttentionBlock(dim=embed_dim, num_heads=num_heads,
                               ffn_ratio=cfg['ffn_ratio'], qkv_bias=cfg['qkv_bias'],
                               proj_bias=cfg['proj_bias'], ffn_bias=cfg['ffn_bias'],
                               norm_layer=norm_layer_cls, act_layer=nn.GELU,
                               ffn_layer=ffn_layer_cls, init_values=cfg['layerscale_init'],
    ...
    def _prepare_tokens(self, x):
        ...
        storage = self.storage_tokens if self.n_storage_tokens > 0 else \
            torch.empty(1, 0, cls_token.shape[-1], dtype=cls_token.dtype, device=cls_token.device)
        x = torch.cat([cls_token.expand(B, -1, -1), storage.expand(B, -1, -1), x], dim=1)
        return x, (H, W)
...
class DINOv3STAs(nn.Module):
    def __init__(self, name='dinov3_vits16', weights_path=None,
                 interaction_indexes=None, finetune=True,
                 conv_inplane=16, hidden_dim=None, patch_size=16, use_sta=True):
        super().__init__()
...
class ConvNormLayer_fuse(nn.Module):
    ...
    def convert_to_deploy(self):
        if not hasattr(self, 'conv_bn_fused'):
            c = self.conv
            self.conv_bn_fused = nn.Conv2d(c.in_channels, c.out_channels, c.kernel_size,
                                           c.stride, groups=c.groups, padding=c.padding, bias=True)
        k = self.conv.weight
...
class VGGBlock(nn.Module):
    ...
    def _fuse(self, branch):
        ...
        rm, rv, g, b, eps = branch.norm.running_mean, branch.norm.running_var, branch.norm.weight, branch.norm.bias, branch.norm.eps
        std = (rv + eps).sqrt()
        t = (g / std).reshape(-1, 1, 1, 1)
...
class CSPLayer2(nn.Module):
    def __init__(self, in_channels, out_channels, num_blocks=3, expansion=1.,
        ...
        self.conv3 = ConvNormLayer_fuse(hidden, out_channels, 1, 1, bias=bias, act=act) \
            if hidden != out_channels else nn.Identity()
...
class HybridEncoder(nn.Module):
    def __init__(self, in_channels=(512, 1024, 2048), feat_strides=(8, 16, 32),
                 hidden_dim=256, nhead=8, dim_feedforward=1024, dropout=0.,
                 enc_act='gelu', use_encoder_idx=(2,), num_encoder_layers=1,
                 pe_temperature=10000, expansion=1., depth_mult=1., act='silu',
                 eval_spatial_size=None, version='deim', csp_type='csp2', fuse_op='sum'):
        super().__init__()
        ...
        encoder_layer = TransformerEncoderLayer(hidden_dim, nhead=nhead,
    ...
    @staticmethod
    def _build_2d_sincos_pos_embed(w, h, embed_dim=256, temperature=10000.):
    ...
    def forward(self, feats):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                if self.training or self.eval_spatial_size is None:
                    pos_embed = self._build_2d_sincos_pos_embed(w, h, self.hidden_dim, self.pe_temperature).to(src.device)
                else:
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            fused = (up + feat_low) if self.fuse_op == 'sum' else torch.cat([up, feat_low], dim=1)
            inner_outs.insert(0, self.fpn_blocks[len(self.in_channels) - 1 - idx](fused))
        outs = [inner_outs[0]]
        for idx in range(len(self.in_channels) - 1):
            feat_low = outs[-1]
            feat_high = inner_outs[idx + 1]
            down = self.downsample_convs[idx](feat_low)
            fused = (down + feat_high) if self.fuse_op == 'sum' else torch.cat([down, feat_high], dim=1)
...
def deformable_attention_core_func_v2(value, value_spatial_shapes, sampling_locations,
    ...
    for level, (h, w) in enumerate(value_spatial_shapes):
        ...
        if method == 'default':
            sv = F.grid_sample(value_l, grid_l, mode='bilinear', padding_mode='zeros', align_corners=False)
        elif method == 'discrete':
            coord = (grid_l * torch.tensor([[w, h]], device=value_l.device) + 0.5).to(torch.int32)
            coord = coord.clamp(0, h - 1).reshape(bs * n_head, Len_q * num_points_list[level], 2)
            s_idx = torch.arange(coord.shape[0], device=value_l.device).unsqueeze(-1).repeat(1, coord.shape[1])
            sv = value_l[s_idx, :, coord[..., 1], coord[..., 0]]
            sv = sv.permute(0, 2, 1).reshape(bs * n_head, c, Len_q, num_points_list[level])
        sampling_value_list.append(sv)
    attn = attention_weights.permute(0, 2, 1, 3).reshape(bs * n_head, 1, Len_q, sum(num_points_list))
    output = (torch.concat(sampling_value_list, dim=-1) * attn).sum(-1).reshape(bs, n_head * c, Len_q)
...
class MSDeformableAttention(nn.Module):
    def __init__(self, embed_dim=256, num_heads=8, num_levels=4, num_points=4,
        ...
        self.ms_deformable_attn_core = functools.partial(deformable_attention_core_func_v2, method=method)
        self._reset_parameters()
        if method == 'discrete':
    ...
    def forward(self, query, reference_points, value, value_spatial_shapes):
        ...
        attention_weights = F.softmax(
            self.attention_weights(query).reshape(bs, Len_q, self.num_heads, sum(self.num_points_list)), dim=-1)
        ...
        return self.ms_deformable_attn_core(value, value_spatial_shapes, sampling_locations,
                                             attention_weights, self.num_points_list)


class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, n_head=8, dim_feedforward=1024, dropout=0.,
                 activation='relu', n_levels=4, n_points=4, cross_attn_method='default',
                 layer_scale=None, use_gateway=False):
        super().__init__()
    ...
    def forward(self, target, reference_points, value, spatial_shapes, attn_mask=None, query_pos_embed=None):
        ...
        target2 = self.cross_attn(_with_pos_embed(target, query_pos_embed),
...
class TransformerDecoder(nn.Module):
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head,
        ...
        self.register_buffer('project', weighting_function(reg_max, up, reg_scale), persistent=False)
    ...
    def forward(self, target, ref_points_unact, memory, spatial_shapes, bbox_head, score_head,
                query_pos_head, pre_bbox_head, integral,
        ...
        return (torch.stack(dec_out_bboxes), torch.stack(dec_out_logits),
                torch.stack(dec_out_corners), torch.stack(dec_out_refs), pre_bboxes, pre_scores)
...
class DEIMTransformer(nn.Module):
    def __init__(self, num_classes=80, hidden_dim=256, num_queries=300,
                 ...
                 num_denoising=100, learn_query_content=False,
                 eval_spatial_size=None, eval_idx=-1, eps=1e-2,
                 cross_attn_method='default', query_select_method='default',
                 reg_max=32, reg_scale=4., layer_scale=1, mlp_act='relu',
        ...
        dec_layer = TransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout,
                                            activation, num_levels, num_points,
                                            cross_attn_method=cross_attn_method, use_gateway=use_gateway)
        dec_layer_wide = TransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout,
                                                  activation, num_levels, num_points,
                                                  cross_attn_method=cross_attn_method,
                                                  layer_scale=layer_scale, use_gateway=use_gateway)
        self.decoder = TransformerDecoder(hidden_dim, dec_layer, dec_layer_wide, num_layers, nhead,
        ...
        self.dec_score_head = nn.ModuleList(
            [dec_score if share_score_head else copy.deepcopy(dec_score) for _ in range(self.eval_idx + 1)] +
        ...
        self.dec_bbox_head = nn.ModuleList(
            [dec_bbox if share_bbox_head else copy.deepcopy(dec_bbox) for _ in range(self.eval_idx + 1)] +
    ...
    def _reset_parameters(self, feat_channels):
        ...
        for w in [self.query_pos_head.layers[0].weight,
    ...
    def _build_input_proj_layer(self, feat_channels):
        ...
        for _ in range(self.num_levels - len(feat_channels)):
            if c == self.hidden_dim:
                ...
                self.input_proj.append(nn.Sequential(OrderedDict([
                    ('conv', nn.Conv2d(c, self.hidden_dim, 3, 2, padding=1, bias=False)),
                    ('norm', nn.BatchNorm2d(self.hidden_dim))])))
                c = self.hidden_dim
    ...
    def _generate_anchors(self, spatial_shapes=None, grid_size=0.05, dtype=torch.float32, device='cpu'):
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            grid_xy = (torch.stack([grid_x, grid_y], -1).unsqueeze(0) + 0.5) / torch.tensor([w, h], dtype=dtype)
            wh = torch.ones_like(grid_xy) * grid_size * (2. ** lvl)
    ...
    def _select_topk(self, memory, outputs_logits, anchors, topk):
        ...
        topk_logits = outputs_logits.gather(1, topk_ind.unsqueeze(-1).repeat(1, 1, outputs_logits.shape[-1])) \
    ...
    def _get_decoder_input(self, memory, spatial_shapes, denoising_logits=None, denoising_bbox_unact=None):
        ...
        topk_mem, topk_logits, topk_anchors = self._select_topk(memory, enc_logits, anchors, self.num_queries)
        ...
        content = self.tgt_embed.weight.unsqueeze(0).tile([memory.shape[0], 1, 1]) \
            if self.learn_query_content else topk_mem.detach()
        topk_bbox_unact = topk_bbox_unact.detach()
        if denoising_bbox_unact is not None:
            topk_bbox_unact = torch.concat([denoising_bbox_unact, topk_bbox_unact], dim=1)
    ...
    def forward(self, feats, targets=None):
        ...
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits = self.decoder(
            content, ref_unact, memory, spatial_shapes,
            self.dec_bbox_head, self.dec_score_head, self.query_pos_head,
            self.pre_bbox_head, self.integral,
            attn_mask=attn_mask)
...
def verify_against_original(checkpoint_path: str, config_path: str,
                             num_classes: int, input_size=(640, 640), atol=1e-4):
    """
    ...
    for name, t_orig, t_new in [('pred_logits', orig_logits, new_logits),
                                  ('pred_boxes',  orig_boxes,  new_boxes)]:
        diff = (t_orig - t_new).abs()
        ok = diff.max().item() <= atol
        print(f"  [{'PASS' if ok else 'FAIL'}] {name:12s}  max_diff={diff.max().item():.2e}  atol={atol}")
...
def main():
    parser = argparse.ArgumentParser(description='DEIMv2-L single-file model — load & verify')
    ...
    if args.config:
        ...
        verify_against_original(args.resume, args.config, args.num_classes,
                                 tuple(args.input_size), args.atol)
    else:
